...
class SphereMenu:UIView, UICollisionBehaviorDelegate {
    ...
    let kSphereLength:CGFloat = 70   //80 changed by me
    let kSphereDamping:Float = 1.2
...
//        self.bounds = CGRectMake(0, 0, 20, 20); //changed by me
    ...
    func commonSetup()
    {
        ...
        for (var i = 0; i < self.count; i++) {
            ...
            item.bounds = CGRectMake(0, 0, 20, 20);   // add by me
...
//        self.itemBehavior?.friction = 0.5;
    ...
    func centerForSphereAtIndex(index:Int) -> CGPoint{
        let firstAngle:CGFloat = CGFloat(M_PI) + (CGFloat(M_PI_2) - kAngleOffset) + CGFloat(index+1) * kAngleOffset
    ...
    func startTapped(gesture:UITapGestureRecognizer){
        ...
        if (self.expanded == true) {
            spinView(self.start, duration: 0.5)
            self.shrinkSubmenu()
        } else {
            spinView(self.start, duration: 0.5)
            self.expandSubmenu()
    ...
    func spinView (view:UIView!, duration:NSTimeInterval)
    {
        let fullRotation = CABasicAnimation(keyPath: "transform.rotation")
        fullRotation.fromValue = NSNumber(floatLiteral: 0)
        fullRotation.toValue = NSNumber(floatLiteral: ((360*M_PI)/180))
